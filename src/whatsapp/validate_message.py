...
def validate_regular_message(original_message):
    ...
    try:
        regular_message = WhatsAppRegularMessageBody.model_validate(original_message)
        ...
        if regular_message.entry[0].changes[0].value.messages[0].type == "audio":
            return True
        if regular_message.entry[0].changes[0].value.messages[0].type == "image":
            return True
...
def validate_template_message(original_message):
    if isinstance(original_message, str):
        original_message = json.loads(original_message)
    try:
        template_message = WhatsAppTemplateMessageBody.model_validate(original_message)
...
def validate_interactive_message(original_message):
    if isinstance(original_message, str):
        original_message = json.loads(original_message)
    try:
        interactive_message = WhatsAppInteractiveMessageBody.model_validate(original_message)
...
def validate_status_message(original_message):
    if isinstance(original_message, str):
        original_message = json.loads(original_message)
    try:
        status_message = WhatsAppStatusMessageBody.model_validate(original_message)
